...
int main(int argc, char** argv) {
    ...
    size_t bytes_A = sizeof(float) * 1024 * 1024;
    size_t bytes_B = sizeof(float) * 1024 * 1024;
    size_t bytes_C = sizeof(float) * 1024 * 1024;
    float* h_A = (float*)malloc(bytes_A);
    float* h_B = (float*)malloc(bytes_B);
    float* h_A1 = (float*)malloc(sizeof(float)*M*K);
    float* h_B1 = (float*)malloc(sizeof(float)*K*N);
    float* h_C = (float*)malloc(bytes_C);
    float* h_C1 = (float*)malloc(sizeof(float)*M*N);
    ...
    checkCudaErrors(cudaMalloc(&d_A1, sizeof(float)*M*K));
    checkCudaErrors(cudaMalloc(&d_B1, sizeof(float)*K*N));
    checkCudaErrors(cudaMalloc(&d_C1, sizeof(float)*M*N));
    ...
    checkCudaErrors(cudaMemcpy( d_A1, h_A1, sizeof(float)*M*K, cudaMemcpyHostToDevice));
    checkCudaErrors(cudaMemcpy( d_B1, h_B1, sizeof(float)*K*N, cudaMemcpyHostToDevice));
    ...
    for (int run = 0 ; run < nIter; run ++ ) {
        ...
        <<< dimGrid, dimBlock >>>(d_A, d_B, d_C, 1024, 1024, 1024);
    }
    ...
    checkCudaErrors(cudaMemcpy( h_C1, d_C1, M*N*sizeof(float), cudaMemcpyDeviceToHost));
    ...
    for(int i=0;i<M;++i)
    {
        for(int j=0;j<N;++j)
        {
                double abs_err = fabs(h_C[i*1024+j] - h_C1[i*N+j]);
                double dot_length = M;
                double abs_val = fabs(h_C[i*1024+j]);
                double rel_err = abs_err / abs_val / dot_length;
                if (rel_err > eps) {
                    printf("Error! Matrix[%05d]=%.8f, ref=%.8f error term is > %E\n",
                            i*1024+j, h_C[i*1024+j], h_C1[i*N+j], eps);
                    correct = false;
                    break;
